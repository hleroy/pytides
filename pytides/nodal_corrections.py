...
def f_K1(a):
    ...
    return (0.2523 * np.sin(2 * I)**2 +
            0.1689 * np.sin(2 * I) * np.cos(nu) + 0.0283)**(0.5) / mean
...
def f_K2(a):
    ...
    return (0.2523 * np.sin(I)**4 +
            0.0367 * np.sin(I)**2 * np.cos(2 * nu) + 0.0013)**(0.5) / mean
...
def u_M1(a):
    I = np.radians(a['I'].value)
    P = np.radians(a['P'].value)
    Q = np.degrees(np.arctan((5 * np.cos(I) - 1) /
                             (7 * np.cos(I) + 1) * np.tan(P)))
